...
const compareTriplets = (aCategories, bCategories) => {
  let aPoints = 0, bPoints = 0
  aCategories.forEach((aCategory, idx) => {
    if(aCategory > bCategories[idx]) aPoints++
    else if(aCategory < bCategories[idx]) bPoints++
  })
...
const diagonalDifference = (arrays) => {
  let rightToLeftSum = arrays[0][2] + arrays[1][1] + arrays [2][0];
  let leftToRightSum = arrays[0][0] + arrays[1][1] + arrays[2][2];
...
const plusMinus = nums => {
  ...
  ratios.push((positives / nums.length).toFixed(6).toString())
  ratios.push((negatives / nums.length).toFixed(6).toString())
  ratios.push((zeros / nums.length).toFixed(6).toString())
  return ratios
}
...
const staircase = n => {
  ...
  for(let i = 0; i < n; i++) {
    let stair = ''
    for(let j = 0; j < n; j++) {
      if(n - j <= i) stair = ' ' + stair 
      else stair = '#' + stair 
    }
    stairs.unshift(stair)
  }
  return stairs
}
...
// Sort array
// Loop through sorted array incrementing both min and max sums as appropriate 
const miniMaxSum = (nums) => {
  nums.sort()
  let minSum = 0, maxSum = 0
  nums.forEach((num, idx) => {
    if(idx !== 0) maxSum += num
    if(idx !== nums.length - 1) minSum += num
  })
...
// If last 2 are AM... 
//   If hour is 12, return hour 00 and minutes and seconds stay the same
//   Else, return the string as is
// Else
//   If hour is 12, return the string as is
//   Else, convert hour to int and Add 12 to hour and reassemble string

const timeConversion = time24hr => {
  const lastTwoChars = time24hr.slice(time24hr.length - 2)
  const newString = time24hr.slice(0, time24hr.length - 2)
  const hoursMinutesSeconds = newString.split(':')
  if(lastTwoChars === 'AM') {
    if(hoursMinutesSeconds[0] === '12') return '00:' + hoursMinutesSeconds[1] + ':' + hoursMinutesSeconds[2]
    else return newString
  } else {
    if(hoursMinutesSeconds[0] === '12') return newString
    else {
      const newHour = parseInt(hoursMinutesSeconds[0]) + 12
      return newHour + ':' + hoursMinutesSeconds[1] + ':' + hoursMinutesSeconds[2]  
    }
  }
}
...
// if current value is not less than 38, then find the next multiple of 5 and the difference between that number and the current number
// if the different is less than 3, set the number to the next multiple of 5
// return the array

const gradingStudents = scores => {
  const roundedScores = scores.map(score => {
    if(score >= 38 && score % 5 !== 0) {
      let nextMultipleOfFive = score
      while(nextMultipleOfFive % 5 !== 0) {
        nextMultipleOfFive++
      }
      if(nextMultipleOfFive - score < 3) return nextMultipleOfFive
    }
    return score
  })
...
const balancedBrackets = str => {
  ...
  const isMatching = (char, prevChar) => {
    if(char === ')' && prevChar === '(') return true
    else if(char === ']' && prevChar === '[') return true
    else if(char === '}' && prevChar === '{') return true
    else return false
  }
  ...
  chars.forEach((char,idx) => {
    if(!charStack.length) charStack.push(char)
    else {
      if(isMatching(char, charStack[idx-1])) charStack.pop()
      else charStack.push(char)
    }
    //console.log("Char stack", charStack);
  })
  if(!charStack.length) return true
  return false
}
...
const isWinningTicket = arrays => {
  const booleans = []
  arrays.forEach(array => {
    let hasEqualCodes = false;
    for(let i in array[0]) {
      if(array[0].charCodeAt(i) === array[1]) hasEqualCodes = true
    }
    booleans.push(hasEqualCodes)
  })
  if(booleans.includes(false)) return false
  else return true
}
...
const gridTrip = (coords, moves) => {
  ...
  const regex = /[A-Z]\d{1,}/g
  const splitMoves = moves.match(regex)

  splitMoves.forEach(move => {
    const numMoves = parseInt(move.slice(1))
    if(move[0] === 'U') finalCoords[1] += numMoves
    else if(move[0] === 'D') finalCoords[1] -= numMoves
    else if(move[0] === 'R') finalCoords[0] += numMoves
    else finalCoords[0] -= numMoves
  })
